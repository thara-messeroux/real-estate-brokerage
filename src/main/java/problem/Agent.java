...
/**
 * Class Agent stores information about an agent. Agents are people responsible for adding listings
 * to the company site.
 *
...
public class Agent<C extends AbstractContract, P extends AbstractProperty> implements IAgent<C, P> {
   ...
   * @param collectionOfTheirCurrentListing - Array List, representing the collection of the agent's
   *                                        current listing.
   * @param commissionRate                  - Double, representing the agent's commission rate,
   *                                        represented as a double between 0 and 1 (inclusive).
   * @param totalEarnings                   - Double, non-negative, representing the total amount
   *                                        the Agent has earned from their sales / rentals.
   */
  public Agent(String name, ArrayList<Listing<C, P>> collectionOfTheirCurrentListing,
  ...
  /**
   * This method validates the agent's commission rate, which is represented as a double between 0
   * and 1 (inclusive).
   *
  ...
  /**
   * This method validates the agent's total earnings which is represented as a non-negative double.
   * This is the total amount the Agent has earned from their sales / rentals.
   *
  ...
  private Double validTotalEarnings(Double validTotalEarnings) {
    if (validTotalEarnings < ZERO) {
      throw new IllegalArgumentException(
          "Invalid Earning Amount. Please enter a non-negative number!");
    }
   ...
   */
  protected Double getListingCommissionValue(Listing listing) {
  ...
  public void dropListing(Listing listing) throws ListingDoesNotExistException {
    if (this.collectionOfTheirCurrentListing.contains(listing)) {
      ...
      throw new ListingDoesNotExistException(
          "The listing to be removed is not included in the collection");
    }
  }
  ...
  /**
   * Sets the name - String, representing the agent's name.
   *
   * @param name - String, representing the agent's name.
   */
  ...
  /**
   * Sets the collectionOfTheirCurrentListing - Array List, representing the collection of the
   * agent's current listing.
   *
   * @param collectionOfTheirCurrentListing - Array List, representing the collection of the agent's
   *                                        current listing.
   */
  public void setCollectionOfTheirCurrentListing(
      ArrayList<Listing<C, P>> collectionOfTheirCurrentListing) {
    this.collectionOfTheirCurrentListing = collectionOfTheirCurrentListing;
  }

  /**
   * Sets the commissionRate - Double, representing the agent's commission rate, represented as a
   * double between 0 and 1 (inclusive).
   *
   * @param commissionRate - Double, representing the agent's commission rate, represented as a
   *                       double between 0 and 1 (inclusive).
   */
  ...
  /**
   * Sets the totalEarnings - Double, non-negative, representing the total amount the Agent has
   * earned from their sales / rentals.
   *
   * @param totalEarnings - Double, non-negative, representing the total amount the Agent has earned
   *                      from their sales / rentals.
   */
  ...
  public boolean equals(Object o) {
    ...
    Agent<?, ?> agent = (Agent<?, ?>) o;
    return getName().equals(agent.getName()) && getCollectionOfTheirCurrentListing()
        .equals(agent.getCollectionOfTheirCurrentListing()) && getCommissionRate()
        .equals(agent.getCommissionRate()) && getTotalEarnings().equals(agent.getTotalEarnings());
  }

  @Override
  public int hashCode() {
    return Objects.hash(getName(), getCollectionOfTheirCurrentListing(), getCommissionRate(),
        getTotalEarnings());
  }
